...
@inline function _quantity_interpolate_kernel(input :: InterpolationInput{3, T}, reference_point :: NTuple{3, T}, ha :: T, LBVH :: LinearBVH, column_idx :: Int, ShepardNormalization :: Bool, :: Type{itpGather}) where {T <: AbstractFloat}
    # Prepare for interpolation
    K = input.smoothed_kernel
    Ktyp = typeof(K)
    Kvalid = KernelFunctionValid(Ktyp, T)
...
@inline function _quantities_interpolate_kernel(input :: InterpolationInput{3, T}, reference_point :: NTuple{3, T}, ha :: T, LBVH :: LinearBVH, columns :: NTuple{M,Int}, ShepardNormalization :: NTuple{M, Bool}, :: Type{itpGather}) where {T <: AbstractFloat, M}
    # Prepare for interpolation
    K = input.smoothed_kernel
    Ktyp = typeof(K)
    Kvalid = KernelFunctionValid(Ktyp, T)
...
@inline function _quantities_interpolate_kernel(input :: InterpolationInput{3, T}, reference_point :: NTuple{3, T}, ha :: T, LBVH :: LinearBVH, itp_strategy :: Type{ITPSTRATEGY} = itpScatter) where {T <: AbstractFloat, ITPSTRATEGY <: AbstractInterpolationStrategy}
    len = length(input.quant)
    columns = ntuple(identity, len)
    ShepardNormalization = ntuple(_ -> true, len)
    if itp_strategy === itpScatter
        return _quantities_interpolate_kernel(input, reference_point, LBVH, columns, ShepardNormalization, itpScatter)
    end
