...
where `h = leaf_scale[leaf_idx]` for leaves and `h = node_scale[node_idx]` for internal nodes.
...
where `h = leaf_scale[leaf_idx]` for leaves and `h = node_scale[node_idx]` for
