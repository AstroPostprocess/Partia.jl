...
Perform CUDA-based SPH interpolation for an unstructured collection of line samples.
This routine prepares the interpolation structures on the CPU, copies particle
input, output grids, and the LBVH to CUDA device memory, evaluates each line
...
- `backend :: CUDAComputeBackend`
  Execution backend specifying CUDA-based interpolation.
...
"""
function Partia.LineSamples_interpolation( :: CUDAComputeBackend, grid_template :: LineSamples{3, TF}, input :: INPUT, catalog :: InterpolationCatalog{3, N, 0, 0, 0, N}, itp_strategy :: Type{ITPSTRATEGY} = itpScatter) where {N, TF <: AbstractFloat, INPUT <: AbstractInterpolationInput{3, TF}, ITPSTRATEGY <: AbstractInterpolationStrategy}
...
Perform CUDA-based SPH interpolation for an unstructured collection of line
...
catalog, copies the required data to CUDA device memory, evaluates each line
sample on the GPU, and copies the interpolated grids back to host memory.
...
"""
function Partia.LineSamples_interpolation( :: CUDAComputeBackend, grid_template :: LineSamples{3, TF}, input :: INPUT, LBVH :: LinearBVH{3, TF}, catalog :: InterpolationCatalog{3, N, 0, 0, 0, N}, itp_strategy :: Type{ITPSTRATEGY} = itpScatter) where {N, TF <: AbstractFloat, INPUT <: AbstractInterpolationInput{3, TF}, ITPSTRATEGY <: AbstractInterpolationStrategy}
    itp_strategy === itpScatter || throw(ArgumentError(
        "LineSamples_interpolation only supports itpScatter. " *
        "Line-integrated samples do not have a well-defined query smoothing length ha, " *
        "so itpGather is not supported."
