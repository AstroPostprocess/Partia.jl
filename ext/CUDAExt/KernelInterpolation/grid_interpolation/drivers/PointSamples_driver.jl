...
Perform SPH interpolation over an arbitrary point-sample grid using CUDA execution.
This routine prepares the interpolation structures on the CPU, copies particle
input, output grids, and the LBVH to CUDA device memory, evaluates each grid
...
Perform SPH interpolation over an arbitrary point-sample grid using CUDA
...
catalog, copies the required data to CUDA device memory, evaluates each grid
point on the GPU, and copies the interpolated grids back to host memory.
...
"""
function Partia.PointSamples_interpolation( :: CUDAComputeBackend, grid_template :: PointSamples{3, TF}, input :: INPUT, LBVH :: LinearBVH{3, TF}, catalog :: InterpolationCatalog{3, N, G, Div, C, L}, itp_strategy :: Type{ITPSTRATEGY} = itpScatter) where {N, G, Div, C, L, TF <: AbstractFloat, INPUT <: AbstractInterpolationInput{3, TF}, ITPSTRATEGY <: AbstractInterpolationStrategy}
