...
function Partia.to_HostVector(input :: InterpolationInput{D, T, V, K, NCOLUMN}) where {D, T <: AbstractFloat, V <: CuVector{T}, K <: AbstractSPHKernel, NCOLUMN}
    return InterpolationInput{D, T, Vector{T}, K, NCOLUMN}(
        input.Npart,
...
function Partia.to_HostVector(AB :: AABB{D, TF, VF}) where {D, TF <: AbstractFloat, VF <: CuVector{TF}}
...
function Partia.to_HostVector(grid :: PointSamples{D, TF, VG, VC}) where {D, TF <: AbstractFloat, VG <: CuVector{TF}, VC <: NTuple{D, CuVector{TF}}}
    return PointSamples(
        Vector{TF}(grid.grid),
...
function Partia.to_HostVector(grid :: LineSamples{D, TF, VG, VC}) where {D, TF <: AbstractFloat, VG <: CuVector{TF}, VC <: NTuple{D, CuVector{TF}}}
    return LineSamples(
        Vector{TF}(grid.grid),
...
function Partia.to_HostVector(grid :: StructuredGrid{D, TF, V, A}) where {D, TF <: AbstractFloat, V <: CuVector{TF}, A <: CuArray{TF, D}}
